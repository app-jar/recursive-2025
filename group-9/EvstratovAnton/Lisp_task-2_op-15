;; Save as: mst.lisp (simple English name!)
...
(defun print-mst (mst)
  ...
  (let ((total 0))
    ...
    (format t "~%Total weight: ~A~%" total)))

;; Test function
(defun test ()
  (format t "=== Prim's Algorithm (Minimum Spanning Tree) ===~%~%")
  
  (format t "Test graph:~%")
  (dolist (e *test-graph*)
    (format t "  ~A-~A (weight: ~A)~%" (edge-from e) (edge-to e) (edge-weight e)))
  
  (format t "~%Result (starting from vertex A):")
